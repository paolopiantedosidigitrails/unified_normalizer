...
from unified_normalizer.Normalizer import Normalizer, SkillNormalizerOld, LanguageNormalizer, \
                                          JobNormalizer, SkillNormalizerNew, JobNormalizerWithHierarchy
import time 
...
def test_word_embedding():
    try:
        ...
        assert len(normalizer.get_word_embedding("test")['embeddings'][0]) == 768
        ...
        vector = normalizer.get_word_embedding("test")['embeddings'][0]
...
def test_cosine_similarity():
    try:
        ...
        vector = normalizer.get_word_embedding("testerino")['embeddings'][0]
...
def test_search_on_normalized_with_notes():
    try:
        ...
        hits = normalizer.vector_search_on_collection(string_to_search= 'test_normalized',
                                                        ...
                                                        limit=20)
        assert len(hits[0]) == 1
        assert hits[0][0]['raw_string'] == 'test_raw'
...
def test_normalize():
    try:
        ...
        assert notes is None
        assert norm == "Mouse"
        assert additional_info['animal_type'] == "rodent"
        assert additional_info['food_flag'] == "no"
...
def test_data_retention():
    try:
        ...
        norm, additional_info, notes, normalization_info = normalizer2.normalize("walrus", threshold_candidate_accept=0.0)
        assert notes is None
        assert norm == "Walrus"
        assert normalization_info['type'] == "candidate_direct_match"
    finally:
...
def test_speed():
    try:
        ...
        t = time.time()
        for x in range(10):
            language_normalizer.normalize("test_language"+str(x), threshold_verified_accept=1.1, threshold_candidate_accept=1.1)
        t = time.time() - t
...
def test_job_role_normalizer_with_hierarchy():
    try:
        ...
        assert notes.get('hierarchical_position') is not None
        assert norm is not None
        assert additional_info['general_cluster'] is not None
        assert additional_info['function'] is not None
        assert norm_info['type'] == 'new_normalized_string'

        norm, additional_info, notes, norm_info = normalizer.normalize('JR. Data scientist')
        assert notes.get('hierarchical_position') is not None
        assert notes['hierarchical_position'] == 'Junior'
        assert norm == "Data Scientist"

        norm, additional_info, notes, norm_info = normalizer.normalize('Data Scientist')
        assert notes.get('hierarchical_position') is not None
        assert notes['hierarchical_position'] == 'Specialist'
        assert norm == "Data Scientist"

        norm, additional_info, notes, norm_info = normalizer.normalize('Head Data Scientist')
        assert notes.get('hierarchical_position') is not None
        assert notes['hierarchical_position'] == 'Head'
        # assert norm == "Data Scientist"
...
def test_batch_embeddings():
    try:
        ...
        normalizer.get_word_embedding("blip")
        ...
        # timing test batch
        list_of_words = [f"test2+{x}" for x in range(100)]
        t = time.time()
        normalizer.get_word_embedding(list_of_words)
        t = time.time() - t
        print(f"100 word embeddings in {t} seconds. single time: {t/100} seconds.")
        assert t < 0.1
        # # fast timing test, from cache
